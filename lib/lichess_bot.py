...
def do_correspondence_ping(
    control_queue: CONTROL_QUEUE_TYPE, period: datetime.timedelta
) -> None:
    """
...
def write_pgn_records(
    pgn_queue: PGN_QUEUE_TYPE, config: Configuration, username: str
) -> None:
...
def logging_configurer(
    level: int, filename: Optional[str], disable_auto_logs: bool
) -> None:
    """
    ...
    if not disable_auto_logs:
        ...
        auto_file_handler = logging.handlers.TimedRotatingFileHandler(
            auto_log_filename,
            delay=True,
            encoding="utf-8",
            when="midnight",
...
def start(
    ...
    correspondence_pinger = multiprocessing.Process(
        ...
        args=(control_queue, seconds(config.correspondence.checkin_period)),
    )
    ...
    logging_listener = multiprocessing.Process(
        ...
        args=(logging_queue, logging_level, log_filename, disable_auto_logging),
    )
    logging_listener.start()

    pgn_queue = manager.Queue()
    pgn_listener = multiprocessing.Process(
        target=write_pgn_records, args=(pgn_queue, config, user_profile["username"])
    )
    pgn_listener.start()
        ...
        while (
            ...
            try:
                ...
                if u_type == "chatLine":
                    ...
                    takeback_field = (
                        game.state.get("btakeback")
                        if game.is_white
                        else game.state.get("wtakeback")
                    )
                    ...
                    if not is_game_over(game) and is_engine_move(
                        ...
                        and li.accept_takeback(
                            game.id, takebacks_accepted < max_takebacks_accepted
                        )
                    ):
...
def fake_think_time(
    config: Configuration, board: chess.Board, game: model.Game
) -> datetime.timedelta:
    ...
    if config.fake_think_time and len(board.move_stack) > 9:
        remaining = max(
            seconds(0), game.my_remaining_time() - msec(config.move_overhead)
        )
...
def final_queue_entries(
    ...
    pgn_queue.put_nowait(
        {"game": {"id": game.id, "pgn": pgn_record, "complete": is_game_over(game)}}
    )
...
def pgn_game_record(
    ...
    try:
        ...
        previous_game_path = get_game_file_path(
            config,
            game.id,
            game.white.name,
            game.black.name,
    ...
    lichess_node: Union[chess.pgn.Game, chess.pgn.ChildNode] = (
...
def get_game_file_path(
    ...
    *,
    ...
    def create_valid_path(s: str) -> str:
        illegal = '<>:"/\\|?*'
        return os.path.join(
            config.pgn_directory, "".join(c for c in s if c not in illegal)
        )
...
def save_pgn_record(event: EventType, config: Configuration, user_name: str) -> None:
    ...
    game_path = get_game_file_path(
        config, game_id, white_name, black_name, user_name, game_is_over
    )
    single_game_path = get_game_file_path(
        config,
        game_id,
        white_name,
        black_name,
...
def start_lichess_bot() -> None:
    ...
    parser.add_argument(
        "--disable_auto_logging", action="store_true", help="Disable automatic logging."
    )
